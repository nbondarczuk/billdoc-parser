...
public:
  ...
  std::string tagvals() const {
    return "FORMHDR" + FS + documentType + FS + customerCode + FS + divertCode +
           FS + firstAddrLine + FS + invoiceDate + FS + referenceNumber + FS +
           customerType + FS + pricesIncVAT + FS + printVAT + FS;
  }
