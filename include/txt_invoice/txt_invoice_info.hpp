...
public:
  ...
  std::string tagvals() const { return "INVCEINFO" + FS + info + FS; }
...
private:
  ...
  const char *DIRECT_DEBIT_INFO =
      "Het totaalbedrag wordt omstreeks %s van uw rekeningnr. %s "
      "afgeschreven.";
  ...
  void make_info() {
    if (paymentMethod == "D") {
      char buffer[256];
      sprintf(buffer, DIRECT_DEBIT_INFO, invoiceDueDate.c_str(),
