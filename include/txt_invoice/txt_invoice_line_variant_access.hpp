...
public:
  TXTInvoiceLineVariantAccess(const std::string &article,
                              ...
                              const XMLDocumentLegend *lgn, std::regex &re)
      : amountNet(0), amountVAT(0), amountWithVAT(0), taxRate(0), price(0),
        charge_item(re) {
  ...
  long GetAmountNet() const { return amountNet; }

  long GetAmountVAT() const { return amountVAT; }

  long GetAmountWithVAT() const { return amountWithVAT; }
...
private:
  ...
  void load_desc(const std::string &article, const XMLDocumentLegend *lgn) {
    TXTInvoiceLineVariant::Description1 =
        lgn->getLongDes("SN", split<5>(article.c_str(), '.')[3].c_str());
    TXTInvoiceLineVariant::Description2 =
        lgn->getLongDes("SP", split<5>(article.c_str(), '.')[2].c_str());
    TXTInvoiceLineVariant::Description3 =
        lgn->getLongDes("TM", split<5>(article.c_str(), '.')[0].c_str());
  }
