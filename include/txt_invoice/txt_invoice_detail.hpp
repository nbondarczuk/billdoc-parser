...
public:
  ...
  std::string tagvals() const {
    return "INVCDETL" + FS + invoiceNumber + FS + contractNumber + FS +
           remittanceOnInvoiceFlag + FS + bankAccountNumber + FS +
           invoiceDueDate + FS + paymentMethod + FS + billingPeriodEndDate +
           FS + contractId + FS + printFlag + FS + eBillFlag + FS;
  }
...
private:
  ...
  void load_address(const XMLDocumentAddressPage *address) {
    ...
    paymentMethod.assign(Decode(address->BillAcc->PayMeth, "Direct Debit", "D",
