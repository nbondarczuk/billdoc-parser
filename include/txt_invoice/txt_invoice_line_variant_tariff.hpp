...
public:
	TXTInvoiceLineVariantTariff(std::string id,
								std::string des,
								std::string start,
								std::string end) :
		tariffId(id),
	...
	void load_access_charges(const XMLDocumentInvoice* invoice,
		...
		std::string pattern("^" + tariffId + "\\.\\d+\\.\\d+\\.\\d+\\.A\\.AC\\.P\\.[PCA]$");
		...
		for (auto it = unique.begin(); it != unique.end(); ++it) {
			...
			TXTInvoiceLineVariantAccess* a1 = new TXTInvoiceLineVariantAccess(*it, invoice, summary, legend, access_past_cred);
			access.push_back(a1);
			accessTotal->Add(a1->GetAmountNet(), a1->GetAmountVAT(), a1->GetAmountWithVAT());
			// advance acess fee
			std::regex access_adv("^" + *it + "\\.AC\\.P\\.[A]$");
			TXTInvoiceLineVariantAccess* a2 = new TXTInvoiceLineVariantAccess(*it, invoice, summary, legend, access_adv);			
			access.push_back(a2);
			accessTotal->Add(a2->GetAmountNet(), a2->GetAmountVAT(), a2->GetAmountWithVAT());
		}
	}
