...
public:
	...
	XMLSumItem(const rapidxml::xml_node<>* node) : root(node) {
		load();
	}	
	...
	void collect_service_dates(std::string& start, std::string& end, std::regex& charge_item) const {
		if (std::regex_match(ArticleString, charge_item)) {
			for (auto it = SrvStatuses.begin(); it != SrvStatuses.end(); ++it) {
				...
				for (auto it2 = status->Dates.begin(); it2 != status->Dates.end(); ++it2) {
...
private:
	...
	void load_charges() {
		load_subnodes<XMLCharge>(root, "Charges", Charges);
	}
	...
	void dump_srv_statuses() const {
		...
		if (n) {
			std::cout << "SrvStatuses: " << "(" << n << ")" << std::endl;		
			for (auto it = SrvStatuses.begin(); it != SrvStatuses.end(); ++it) {
				(*it)->dump();
			}
		}
