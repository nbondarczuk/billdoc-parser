...
inline const char *find_attribute_value(const char *name,
                                        const rapidxml::xml_node<> *root) {
  assert(root);
  assert(name);
  for (rapidxml::xml_attribute<> *a = root->first_attribute(name); a;
       a = a->next_attribute()) {
...
inline const char *
find_attribute_value_option(const char *name,
                            const rapidxml::xml_node<> *root) {
  assert(root);
  assert(name);
  for (rapidxml::xml_attribute<> *a = root->first_attribute(name); a;
...
inline std::vector<char> *read_binary_file(const std::string filename) {
  ...
  vp->insert(vp->begin(), std::istream_iterator<char>(file),
...
T *find_subnode(const rapidxml::xml_node<> *root, const char *name) {
  ...
  assert(name);
  T *ptr = 0;
  for (rapidxml::xml_node<> *node = root->first_node(name); node;
...
template <typename T>
void load_subnodes(const rapidxml::xml_node<> *root, const char *name,
                   std::set<T *> &s) {
  assert(root);
  assert(name);
  for (rapidxml::xml_node<> *node = root->first_node(name); node;
...
template <typename T>
void load_subnodes_dict(const rapidxml::xml_node<> *root, const char *name,
                        const char *index, std::map<const char *, T *> &m) {
  assert(root);
  assert(name);
  assert(index);
  for (rapidxml::xml_node<> *node = root->first_node(name); node;
