...
template <int N> std::string FormatDecimalAmount(std::string a) {
  std::string amount(a);
  amount[a.size() - (N + 1)] = ',';
  return amount;
}

template <int N> std::string FormatDecimalRate(std::string r) {
  std::string rate(r);
  rate[r.size() - (N + 1)] = ',';
  return rate.substr(0, rate.size() - (N + 1));
}

template <int N> long GetDecimalAmount(std::string a) {
  std::string amount(a.substr(0, a.size() - (N + 1)));
  amount += a.substr(a.size() - N, N);
  return atol(amount.c_str());
}

template <int N> std::string PutDecimalAmount(long a) {
  char buffer[256];
  std::string sign;
  if (a == 0) {
    return "0,00";
