...
public:
  ...
  const char *getLongDes(const char *type, const char *id) const {
    ...
    std::map<const std::string, XMLTypeDesc *>::const_iterator it2 =
        TypeDescDict.find(key);
...
private:
  ...
  void load_type_desc_dict() {
    load_subnodes_dict<XMLTypeDesc>(root, "TypeDesc", "Type", "Id",
                                    TypeDescDict);
  }
