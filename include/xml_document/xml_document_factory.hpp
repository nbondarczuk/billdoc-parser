...
public:
  ...
  XMLDocument *make(const char *fn) const {
    ...
    if (it != models.end()) {
      ...
      throw std::invalid_argument(std::string("Can't find model for: ") +
                                  d->id());
    }
...
private:
  ...
  typedef std::map<const std::string, XMLDocument *>::const_iterator
      ModelDictConstIter;
  ModelDict models;

  // register all known document models
  void init_models() {
    models.insert(std::pair<const std::string, XMLDocument *>(
        "Document.Invoice", new XMLDocumentInvoice()));
    models.insert(std::pair<const std::string, XMLDocument *>(
        "Document.AddressPage", new XMLDocumentAddressPage()));
    models.insert(std::pair<const std::string, XMLDocument *>(
        "Document.BalancePage", new XMLDocumentBalancePage()));
    models.insert(std::pair<const std::string, XMLDocument *>(
        "Document.Legend", new XMLDocumentLegend()));
    models.insert(std::pair<const std::string, XMLDocument *>(
        "Document.CallDetails", new XMLDocumentCallDetails()));
    models.insert(std::pair<const std::string, XMLDocument *>(
        "Document.Summary", new XMLDocumentSummary()));
    models.insert(std::pair<const std::string, XMLDocument *>(
        "Document.Complement", new XMLDocumentComplement()));
    models.insert(std::pair<const std::string, XMLDocument *>(
        "Document.Contracts", new XMLDocumentContracts()));
    models.insert(std::pair<const std::string, XMLDocument *>(
        "Document.InvoiceInfo", new XMLDocumentInvoiceInfo()));
    models.insert(std::pair<const std::string, XMLDocument *>(
