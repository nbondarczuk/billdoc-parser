...
public:
	...
	XMLDocumentInvoiceInfo(rapidxml::xml_node<>* node) : root(node) {
		load();
	}	
...
private:
	...
	void load_adv_txt() {
		load_subnodes<XMLAdvTxt>(root, "AdvTxt", AdvTxt);
	}

	void dump_adv_txt() const {
		const unsigned int n = AdvTxt.size();
		if (n) {
			std::cout << "AdvTxt: " << "(" << n << ")" << std::endl;
